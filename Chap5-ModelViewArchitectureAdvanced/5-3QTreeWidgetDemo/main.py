...
from tree_models import OrganizationTreeModel
from tree_controller import OrganizationController
...
def run_quick():
    ...
    # Create model
    model = OrganizationTreeModel()
    
    # Create controller
    controller = OrganizationController(model)
    
    # Expose model and controller to QML
    engine.rootContext().setContextProperty("organizationModel", model)
    engine.rootContext().setContextProperty("orgController", controller)
