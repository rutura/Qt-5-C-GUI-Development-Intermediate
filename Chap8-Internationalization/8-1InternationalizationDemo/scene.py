...
from PySide6.QtGui import QPen, QBrush, QColor, QClipboard, QPixmap, QUndoStack
from painterapptypes import ResizableRectType, ResizableEllipseType, ResizablePixmapType, ResizableStarType, StrokeType
from commands import AddCommand, RemoveCommand, MoveCommand
...
class Scene(QGraphicsScene):
    ...
    def __init__(self, parent=None):
        ...
        # Initialize undo stack for undo/redo functionality
        self.undo_stack = QUndoStack(self)
        
        # For tracking items during move operations
        self.m_selected_items = []
        self.selected_item_initial_positions = []
    ...
    def dropEvent(self, event):
        ...
        if key is not None:
            ...
            if key == 10:  # Ellipse
                ...
                # Use AddCommand instead of direct add
                add_command = AddCommand(ellipse, self)
                ...
                # Use AddCommand instead of direct add
                add_command = AddCommand(pix_item, self)
                self.undo_stack.push(add_command)
                ...
                # Use AddCommand instead of direct add
                add_command = AddCommand(rect_item, self)
                self.undo_stack.push(add_command)
                ...
                # Use AddCommand instead of direct add
                add_command = AddCommand(star_item, self)
                self.undo_stack.push(add_command)
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            if self.tool in [self.Pen, self.Eraser, self.Rect, self.Star, self.Ellipse]:
                ...
                self.drawing = True
            elif self.tool == self.Cursor:
                # Clear selection tracking for move commands
                self.m_selected_items.clear()
                self.selected_item_initial_positions.clear()
    ...
    def mouseMoveEvent(self, event):
        ...
        elif (event.buttons() & Qt.MouseButton.LeftButton) and (self.tool == self.Cursor):
            # Track selected items for move command
            if not self.m_selected_items and not self.selected_item_initial_positions and self.selectedItems():
                self.m_selected_items = self.selectedItems()
                
                for item in self.m_selected_items:
                    self.selected_item_initial_positions.append(item.scenePos())
    ...
    def mouseReleaseEvent(self, event):
        ...
        if (event.button() == Qt.MouseButton.LeftButton) and self.drawing:
            ...
            if self.tool == self.Pen:
                ...
                # Use AddCommand instead of direct add
                add_command = AddCommand(m_rect, self)
                self.undo_stack.push(add_command)
                ...
                # Use AddCommand instead of direct add
                add_command = AddCommand(ellipse_item, self)
                self.undo_stack.push(add_command)
        ...
        elif (event.button() == Qt.MouseButton.LeftButton) and (self.tool == self.Cursor):
            # Create move commands for moved items
            for i in range(len(self.m_selected_items)):
                current_item = self.m_selected_items[i]
                
                if self.selected_item_initial_positions[i] != current_item.scenePos():
                    move_command = MoveCommand(
                        current_item, 
                        self,
                        self.selected_item_initial_positions[i],
                        current_item.scenePos()
                    )
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_Delete:
            if len(self.selectedItems()) > 0:
                ...
                # Use RemoveCommand instead of direct remove
                remove_command = RemoveCommand(item_to_remove, self)
    ...
    def draw_line_to(self, end_point):
        ...
        if not self.line_group:
            ...
            # Use AddCommand instead of direct add
            add_command = AddCommand(self.line_group, self)
            self.undo_stack.push(add_command)
